...
fn run_chat(args: ChatArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::var("VF_QWEN35_GDN_VERIFY").as_deref() == Ok("1") {
        ...
        if std::env::var("VF_QWEN35_GDN_TYPEBISECT").as_deref() == Ok("1") {
            forward.gdn_type_bisect(&dev, &registry, &cmd_ctx, &model)?;
        }
        forward.destroy(&dev.device, &mut allocator);
        return Ok(());
    }
...
fn run_chat_safetensors(args: ChatArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // prefill is now an opt-in regression-bisect path; set
    // `VULKANFORGE_FORCE_PER_TOKEN=1` to keep the v0.3.14 path for
    // comparison.
    //
    // Sprint 46F — the prior Gemma-4 auto-on (Sprint 43F) is removed:
    // F32 mul_mm SPVs (46B), batch PleBlock plumbing (46D), and
    // Gemma-4-aware batch flash-attn (46E) make the batch path
    // mathematically valid. Bit-ID smoke against the env-var path is
    // documented in results/v0315_sprint46f_*.md.
...
fn inference_support(arch: &str, file_type: Option<u32>) -> (bool, bool) {
    // Sprint 17A — `llama` joins `qwen2` / `qwen3` after pre-check
    // confirmed every architecture-specific concern (RoPE variant,
    // q/k-norm presence, FFN/vocab dims) is already auto-detected
    // from the GGUF metadata + tensor presence; the loader and
    // forward pass were already generic over tensor names. The
    ...
    //   17 — Q5_K_M  (Q5_K bulk + Q6_K attn_v/ffn_down/output)
    //
    // Sprint 17B added Q3_K shaders. Sprint 17C added Q5_K shaders,
    // which simultaneously fixed Q3_K_M and unlocked Q5_K_S / Q5_K_M.
    //
    // Sprint 17D added Q4_0 shader infrastructure (decode GEMV + Mmq
    // prefill). The shaders are bit-exact vs CPU
    // (tests/q4_0_gemv_correctness.rs, 2/2 pass) but file_type=2 stays
    // gated out: every Q4_0 GGUF in ~/models is Qwen2.5, which needs
    // architectural features VulkanForge doesn't ship yet —
    // attn_q/k/v.bias add (all Qwen2.5 variants), Q4_1 shader
