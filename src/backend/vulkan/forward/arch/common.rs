...
pub(crate) fn layer_weight_shader(model: &LoadedModel, layer: u32, suffix: &str, subgroup: bool) -> ShaderId {
    ...
    match (ggml_type, subgroup) {
        ...
        // Sprint G-5 — when both `subgroup` is on AND `VF_GEMV_NO_SHMEM=1`,
        // pick the Path C no-shmem variant for K-quants (matches what
        // llama.cpp routes to on AMD non-GCN by default).
        (GgmlType::Q6K, true ) if crate::backend::vulkan::pipeline_registry::gemv_no_shmem_enabled() => ShaderId::MulMatVecQ6KSubgroupNoShmem,
        (GgmlType::Q6K, true ) => ShaderId::MulMatVecQ6KSubgroup,
        (GgmlType::Q6K, false) => ShaderId::MulMatVecQ6K,
        (GgmlType::Q3K, true ) if crate::backend::vulkan::pipeline_registry::gemv_no_shmem_enabled() => ShaderId::MulMatVecQ3KSubgroupNoShmem,
        (GgmlType::Q3K, true ) => ShaderId::MulMatVecQ3KSubgroup,
        (GgmlType::Q3K, false) => ShaderId::MulMatVecQ3K,
        (GgmlType::Q5K, true ) if crate::backend::vulkan::pipeline_registry::gemv_no_shmem_enabled() => ShaderId::MulMatVecQ5KSubgroupNoShmem,
        (GgmlType::Q5K, true ) => ShaderId::MulMatVecQ5KSubgroup,
        (GgmlType::Q5K, false) => ShaderId::MulMatVecQ5K,
        (GgmlType::Q4K, true ) if crate::backend::vulkan::pipeline_registry::gemv_no_shmem_enabled() => ShaderId::MulMatVecQ4KSubgroupNoShmem,
