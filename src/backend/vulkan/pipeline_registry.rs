...
impl PipelineRegistry {
    ...
    pub fn new(
        ...
        // Sprint 16C — only load the Q4_K naive coopmat shaders (which
        // declare FP8/BF16 SPIR-V capabilities) when the user has
        // opted into the Sprint-3 coopmat path. Skipping them by
        // default removes 16 validation-layer false-negatives per
        // session because ash 0.38 cannot enable the corresponding
        // VK_EXT_shader_float8 / VK_KHR_shader_bfloat16 features.
        let coopmat_q4k_opt_in = std::env::var("VULKANFORGE_COOPMAT")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);
        let extra_shaders: &[ShaderId] = if coopmat_q4k_opt_in {
            shaders::COOPMAT_Q4K_OPTIONAL_SHADERS
        } else {
            &[]
        };
        for &id in shaders::ALL_SHADERS.iter().chain(extra_shaders.iter()) {
