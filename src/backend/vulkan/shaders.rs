...
pub enum ShaderId {
    ...
    MulCoopmatFp8Bn32,
    /// Sprint 33 — BN=64 variant. 16 Wave64 subgroups arranged
    /// 4 × 4 (M × N), 64 × 64 output tile per WG. BLOCK_SIZE=1024
    /// is the RDNA4 maximum workgroup size. Fourth doubling of
    /// activation reuse per A-tile load vs naive. Trade-off:
    /// occupancy drops from ~5 WGs/CU (BN=32, LDS-limited) to
    ...
    /// barrier per layer.
    SwiGLU,
    /// Sprint 43D-2 — GELU(pytorch_tanh)-GLU for Gemma-4: same shape
    /// + bindings as SwiGLU, only the activation on `gate` is the
    ...
    /// (src0 rolling-window input, src1 conv kernel `[nc, nr]` per-channel,
    /// dst output `[nr, n_t, n_s]`); 11 u32 push consts (nb01, nb02,
    /// nb11, dst_nb0, dst_nb1, dst_nb2, nc, ncs, nr, n_t, n_s). Spec
    ...
    /// Sprint 10c — row_split=4 coopmat FA, hd256, f16 K/V direct-global
    /// (occ-12 kernel; v1 floor). Q f32, K/V f16.
    ...
    MulMmQ4KAligned,
    MulMmQ6KAligned,
    // Sprint 46B — F32×F32 mul_mm. Built from mul_mm.comp with
    // DATA_A_F32 (LOAD_VEC_A=1 unaligned, =4 aligned). Routes Gemma-4
    // SafeTensors batch-prefill weights through a real F32 GEMM
    // instead of the Q4_K coopmat shader (which interpreted the
    // bytes as Q4_K blocks and produced NaN — Sprint 43F root cause).
    // Wired into BatchExec in Sprint 46C.
    MulMmF32,
    MulMmF32Aligned,
    ...
    /// Sprint v0.5.2 — Path C (`USE_SUBGROUP_ADD_NO_SHMEM`) indexed
    /// (MUL_MAT_ID) decode expert-GEMV. Faithful to llama's gfx1201
    /// decode path (`reduc16 = SHADER_REDUCTION_MODE_SUBGROUP` →
    /// `_subgroup_no_shmem` SPV) vs VF's HYBRID `_id_subgroup`
    /// (`USE_SUBGROUP_ADD`). Registered with NUM_ROWS=2 (rm_kq) and
    /// selected via `VF_GEMV_ID_NO_SHMEM=1`.
    MulMatVecQ3KIdSubgroupNoShmem,
    MulMatVecQ4KIdSubgroupNoShmem,
    MulMatVecQ5_0IdSubgroupNoShmem,
    MulMatVecQ4_0IdSubgroupNoShmem,
...
impl ShaderId {
    pub fn name(self) -> &'static str {
        match self {
            ...
            ShaderId::MulMmF32 => "mul_mm_f32",
            ShaderId::MulMmF32Aligned => "mul_mm_f32_aligned",
            ...
            ShaderId::MulMatVecQ3KIdSubgroupNoShmem => "mul_mat_vec_q3_k_f32_f32_id_subgroup_no_shmem",
            ShaderId::MulMatVecQ4KIdSubgroupNoShmem => "mul_mat_vec_q4_k_f32_f32_id_subgroup_no_shmem",
            ShaderId::MulMatVecQ5_0IdSubgroupNoShmem => "mul_mat_vec_q5_0_f32_f32_id_subgroup_no_shmem",
            ShaderId::MulMatVecQ4_0IdSubgroupNoShmem => "mul_mat_vec_q4_0_f32_f32_id_subgroup_no_shmem",
...
pub const ALL_SHADERS: &[ShaderId] = &[
    ...
    // Sprint 46B — F32 mul_mm: always-instantiated like the K-quant
...
pub const FLASH_ATTN_CM_GEMMA_RS_HD256: &[u8] =
...
// Sprint 46B — F32×F32 mul_mm SPVs (DATA_A_F32 path).
pub const MUL_MM_F32: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mm_f32.spv"));
pub const MUL_MM_F32_ALIGNED: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mm_f32_aligned.spv"));
...
// Sprint v0.5.2 — Path C indexed (no_shmem) decode expert-GEMV variants.
pub const MUL_MAT_VEC_Q3_K_F32_F32_ID_SUBGROUP_NO_SHMEM: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mat_vec_q3_k_f32_f32_id_subgroup_no_shmem.spv"));
pub const MUL_MAT_VEC_Q4_K_F32_F32_ID_SUBGROUP_NO_SHMEM: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mat_vec_q4_k_f32_f32_id_subgroup_no_shmem.spv"));
pub const MUL_MAT_VEC_Q5_0_F32_F32_ID_SUBGROUP_NO_SHMEM: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mat_vec_q5_0_f32_f32_id_subgroup_no_shmem.spv"));
pub const MUL_MAT_VEC_Q4_0_F32_F32_ID_SUBGROUP_NO_SHMEM: &[u8] =
    include_bytes!(concat!(env!("OUT_DIR"), "/mul_mat_vec_q4_0_f32_f32_id_subgroup_no_shmem.spv"));
