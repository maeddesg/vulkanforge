...
| Batched Full-attention prefill layers (**default OFF**, v0.5.9) | `VF_PREFILL_FULL_BATCHED=1` | Gemma-4-26B-A4B (MoE) | Keeps the 5 Full-attention layers on the batched prefill path instead of the 51D-AN per-token workaround (which costs 1 GPU drain per token per Full layer). **+58 % / +47 % prefill** (201 → 317 t/s @p512, 181 → 266 @p2048). Full gate battery green (bit-identical to an independent per-query attention reference; recall/5-smoke/multichunk/273-tests). Default stays the workaround pending the owner default-flip decision. |
