...
| qwen35 batched prefill (**opt-in** since v0.5.5) | `VF_QWEN35_BATCHED=1` | Qwen3.6-27B-MTP (qwen35) | Routes qwen35 prefill through the batched executor (GDN + Full-Attn over seq_len=N; cross-token cores serial with state-carry, projections batched GEMM). **4–11× prefill speedup** (grows with prompt length), value-preserving (greedy output byte-identical to per-token), decode-neutral. Default-OFF = unchanged per-token prefill. |
