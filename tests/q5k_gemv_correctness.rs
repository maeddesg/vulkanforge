//! Sprint 17C — Q5_K GPU GEMV correctness check.
//!
//! Mirror of `tests/q3k_gemv_correctness.rs` for Q5_K. Loads real
//! Q5_K weights from `blk.0.attn_v.weight` in the Q3_K_M GGUF,
//! dispatches MulMatVecQ5K (stock + subgroup) on GPU with input=ones,
//! and compares against a CPU reference computed via
//! `q5k::dequant_block` + scalar dot product.
...
fn run_gpu_vs_cpu(shader: ShaderId, label: &str) {
    ...
    // M=4 rows × K=4096 cols (= 16 Q5_K blocks per row). Mirrors
    // a real Qwen3-8B GEMV (hidden=4096) and exercises the
    // `it_size > 1` main-loop path of mul_mat_vec_q5_k.comp.
    const M: usize = 4;
    const K: usize = 4096;
    let blocks_per_row = K / q5k::QUANT_K;
    let bytes_per_row = blocks_per_row * q5k::BLOCK_BYTES;
    ...
    for r in 0..M {
        for b in 0..blocks_per_row {
            let off = r * bytes_per_row + b * q5k::BLOCK_BYTES;
            let block: &[u8; q5k::BLOCK_BYTES] = weights_bytes
                [off..off + q5k::BLOCK_BYTES]
                .try_into()
                .unwrap();
            let dq = q5k::dequant_block(block);
            for k in 0..q5k::QUANT_K {
                cpu_out[r] += dq[k] * input[b * q5k::QUANT_K + k];
            }
        }
    }
...
#[test]
fn q5k_gemv_stock_matches_cpu() {
    run_gpu_vs_cpu(ShaderId::MulMatVecQ5K, "stock");
}

#[test]
fn q5k_gemv_subgroup_matches_cpu() {
    run_gpu_vs_cpu(ShaderId::MulMatVecQ5KSubgroup, "subgroup");
}
