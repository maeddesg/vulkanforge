...
// ----------------------------------------------------------------
// Sprint 17B follow-up — Q3_K Mmq GPU vs CPU correctness.
//
// Mirror of test_gemm_q4k_full_tile_64x64_mul_mmq for Q3_K. Uses
// real Q3_K weights from token_embd.weight in the Q3_K_M GGUF
// (we don't have a Q3_K encoder). If GPU output diverges from CPU
// reference by more than Q8_1 round-off, the bug is inside
// MulMmqQ3K compute (or its LDS / shared-memory layout).
...
fn cpu_gemm_q3k_ref(
    ...
    use vulkanforge::backend::vulkan::q3k::{BLOCK_BYTES, QUANT_K, dequant_block};
    assert_eq!(k % QUANT_K, 0);
    let bpr = k / QUANT_K;
    let mut out = vec![0.0f32; m * n];
    let mut dq_rows: Vec<f32> = vec![0.0f32; m * k];
    ...
    // Output layout per mul_mmq.comp: data_d[col * stride_d + row], stride_d = M.
    for col in 0..n {
        for row in 0..m {
            let mut acc = 0.0f64;
            for e in 0..k {
...
fn run_mul_mmq_q3k_parity(m: u32, n: u32, k: u32, label: &str) {
    use std::path::PathBuf;
    use vulkanforge::backend::vulkan::gguf::{GgmlType, GgufFile};
    use vulkanforge::backend::vulkan::q3k;

    // Load real Q3_K weights from token_embd.weight in the
    // Q3_K_M GGUF. Skip cleanly if the model isn't present.
    let home = std::env::var("HOME").expect("HOME unset");
    let gguf_path = PathBuf::from(&home).join("models/Qwen3-8B-Q3_K_M.gguf");
    if !gguf_path.exists() {
        eprintln!("skip [{label}] — {} not found", gguf_path.display());
        return;
    }
    let gguf = GgufFile::open(&gguf_path).expect("open gguf");
    let info = gguf.tensor("token_embd.weight").expect("token_embd.weight");
    assert_eq!(info.ggml_type, GgmlType::Q3K);
    let all_bytes = gguf.tensor_bytes(info);

    assert_eq!(k as usize % q3k::QUANT_K, 0);
    let bytes_per_row = (k as usize / q3k::QUANT_K) * q3k::BLOCK_BYTES;
    let needed = (m as usize) * bytes_per_row;
    assert!(all_bytes.len() >= needed, "GGUF too small for M={m} K={k}");
    let weights: Vec<u8> = all_bytes[..needed].to_vec();
    ...
    let w_buf = {
        ...
        let mut b = GpuBuffer::new(
            &device, allocator, weights.len() as u64,
            vk::BufferUsageFlags::STORAGE_BUFFER, MemoryLocation::CpuToGpu, "q3k_w",
        ).expect("alloc weights");
        b.write_bytes(&weights).expect("write weights");
        fix.track(b)
    };
    eprintln!("[{label}] M={m} N={n} K={k} blocks={}", k as usize / q3k::QUANT_K);

    // N tokens × K features each, deterministic non-trivial values.
    let acts: Vec<f32> = (0..(n * k))
        .map(|i| ((i as f32) * 0.013).sin() * 0.5)
        .collect();
    let act_buf = fix.host_buffer_f32(&acts, "q3k_act");

    // Q8_1 quantization for mul_mmq.
    let q8_blocks_x4 = ((n * k + 127) / 128) as usize;
    let q8_buf = {
        let device = fix.dev.device.clone();
        let allocator = fix.allocator.as_mut().unwrap();
        let buf = GpuBuffer::new(
            &device, allocator, (q8_blocks_x4 * 144) as u64,
            vk::BufferUsageFlags::STORAGE_BUFFER, MemoryLocation::GpuOnly, "q3k_q8",
        ).expect("alloc q8");
        fix.track(buf)
    };
    let q8_pc = Q8_1QuantizePushConstants { ne: n * k, num_blocks: q8_blocks_x4 as u32 };
    ...
    let out_buf = fix.output_buffer_f32((m * n) as usize, "q3k_out");
    let pc = MmqPushConstants {
        m, n, k,
        stride_a: k,
        stride_b: k,
    ...
    // S-tile (BM=BN=64): groups = (ceil(M/64), ceil(N/64), 1).
    let groups_x = (m + 63) / 64;
    let groups_y = (n + 63) / 64;
    fix.dispatch(
        ShaderId::MulMmqQ3K, &[w_buf, q8_buf, out_buf],
        bytemuck::bytes_of(&pc),
        (groups_x, groups_y, 1),
    );

    let gpu = fix.read_output(out_buf, (m * n) as usize);
    let cpu = cpu_gemm_q3k_ref(&weights, &acts, m as usize, n as usize, k as usize);

    let nan = gpu.iter().any(|x| !x.is_finite());
    let cpu_amax = cpu.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
    let gpu_amax = gpu.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
    eprintln!(
        "[{label}] cpu_amax={cpu_amax:.4e} gpu_amax={gpu_amax:.4e} nan={nan}"
    );
    eprintln!("  CPU first 8: {:?}", &cpu[..8]);
    eprintln!("  GPU first 8: {:?}", &gpu[..8]);

    assert!(!nan, "[{label}] MulMmqQ3K produced NaN/Inf — first 8: {:?}", &gpu[..8]);

    let max_err = max_abs_err(&gpu, &cpu);
    let thresh = (cpu_amax * 0.05).max(0.5);

    if max_err >= thresh {
        eprintln!("[{label}] FAIL — per-col max errors:");
        let mut col_errs = vec![0.0f32; n as usize];
        let mut col_amax = vec![0.0f32; n as usize];
        for col in 0..n as usize {
            for row in 0..m as usize {
                let idx = col * m as usize + row;
                col_errs[col] = col_errs[col].max((gpu[idx] - cpu[idx]).abs());
                col_amax[col] = col_amax[col].max(cpu[idx].abs());
            }
        }
        for c in 0..(n as usize).min(8) {
            eprintln!("  col {c:>2}: err={:.4e}  cpu_amax={:.4e}  ratio={:.3}",
                col_errs[c], col_amax[c], col_errs[c] / col_amax[c].max(1e-9));
        }
        if n as usize > 16 {
            eprintln!("  ...");
            for c in (n as usize - 4)..n as usize {
                eprintln!("  col {c:>2}: err={:.4e}  cpu_amax={:.4e}  ratio={:.3}",
                    col_errs[c], col_amax[c], col_errs[c] / col_amax[c].max(1e-9));
            }
        }
        // Also: row-wise distribution
        eprintln!("[{label}] per-row max errors (first 8 + last 8):");
        let mut row_errs = vec![0.0f32; m as usize];
        let mut row_amax = vec![0.0f32; m as usize];
        for col in 0..n as usize {
            for row in 0..m as usize {
                let idx = col * m as usize + row;
                row_errs[row] = row_errs[row].max((gpu[idx] - cpu[idx]).abs());
                row_amax[row] = row_amax[row].max(cpu[idx].abs());
            }
        }
        for r in 0..(m as usize).min(8) {
            eprintln!("  row {r:>4}: err={:.4e} cpu_amax={:.4e}", row_errs[r], row_amax[r]);
        }
        if m as usize > 16 {
            eprintln!("  ...");
            for r in (m as usize - 8)..m as usize {
                eprintln!("  row {r:>4}: err={:.4e} cpu_amax={:.4e}", row_errs[r], row_amax[r]);
            }
        }
    }
    assert!(
        max_err < thresh,
        "[{label}] MulMmqQ3K vs CPU max_err = {max_err:e} >= {thresh:e} (cpu_amax={cpu_amax:e}, gpu_amax={gpu_amax:e})"
    );
    fix.teardown();
}

#[test]
fn test_gemm_q3k_full_tile_64x64_mul_mmq() {
    use vulkanforge::backend::vulkan::q3k;
    run_mul_mmq_q3k_parity(64, 64, q3k::QUANT_K as u32, "M=64 N=64 K=256");
}

// Sprint 17B-debug: K=4096 (16 blocks/row) — this is what real
// layer matmuls use (hidden_dim=4096 in Qwen3-8B). The 64×64 K=256
// test passes; if THIS one fails, the bug is K-dimension dependent
// (LDS double-loading, k_step iteration, or BK_STEP unrolling).
#[test]
fn test_gemm_q3k_64x64_k4096_mul_mmq() {
    run_mul_mmq_q3k_parity(64, 64, 4096, "M=64 N=64 K=4096");
}

// Sprint 17B-debug: M=4096, n=32, k=4096 — actual chat prefill
// shape for attn_q in Qwen3-8B. If this fails, we have the smoking
// gun for the chat "!!!" output.
#[test]
fn test_gemm_q3k_realistic_prefill() {
    run_mul_mmq_q3k_parity(4096, 32, 4096, "real prefill");
}

// ----------------------------------------------------------------
// Sprint 17C — Q5_K Mmq GPU vs CPU correctness.
//
// Mirror of the Q3_K Mmq parity tests. Loads real Q5_K weights from
// blk.0.attn_v.weight in the Q3_K_M GGUF (Q5_K is the bug-trigger
// from Sprint 17B), dispatches MulMmqQ5K against Q8_1 activations,
// and compares to a CPU reference computed via q5k::dequant_block.
...
fn cpu_gemm_q5k_ref(
    ...
    use vulkanforge::backend::vulkan::q5k::{BLOCK_BYTES, QUANT_K, dequant_block};
    assert_eq!(k % QUANT_K, 0);
    let bpr = k / QUANT_K;
    let mut out = vec![0.0f32; m * n];
    let mut dq_rows: Vec<f32> = vec![0.0f32; m * k];
...
fn run_mul_mmq_q5k_parity(m: u32, n: u32, k: u32, label: &str) {
    use std::path::PathBuf;
    use vulkanforge::backend::vulkan::gguf::{GgmlType, GgufFile};
    use vulkanforge::backend::vulkan::q5k;
    ...
    let gguf = GgufFile::open(&gguf_path).expect("open gguf");
    // attn_v.weight in Q3_K_M is Q5_K — that's why we use this tensor.
    let info = gguf.tensor("blk.0.attn_v.weight").expect("blk.0.attn_v.weight");
    assert_eq!(info.ggml_type, GgmlType::Q5K);
    let all_bytes = gguf.tensor_bytes(info);

    assert_eq!(k as usize % q5k::QUANT_K, 0);
    let bytes_per_row = (k as usize / q5k::QUANT_K) * q5k::BLOCK_BYTES;
    let needed = (m as usize) * bytes_per_row;
    assert!(all_bytes.len() >= needed, "GGUF too small for M={m} K={k}");
    let weights: Vec<u8> = all_bytes[..needed].to_vec();
    ...
    let w_buf = {
        ...
        let mut b = GpuBuffer::new(
            &device, allocator, weights.len() as u64,
            vk::BufferUsageFlags::STORAGE_BUFFER, MemoryLocation::CpuToGpu, "q5k_w",
        ).expect("alloc weights");
        b.write_bytes(&weights).expect("write weights");
        fix.track(b)
    };
    eprintln!("[{label}] M={m} N={n} K={k} blocks={}", k as usize / q5k::QUANT_K);

    let acts: Vec<f32> = (0..(n * k))
        .map(|i| ((i as f32) * 0.013).sin() * 0.5)
        .collect();
    let act_buf = fix.host_buffer_f32(&acts, "q5k_act");
    ...
    let q8_buf = {
        ...
        let buf = GpuBuffer::new(
            &device, allocator, (q8_blocks_x4 * 144) as u64,
            vk::BufferUsageFlags::STORAGE_BUFFER, MemoryLocation::GpuOnly, "q5k_q8",
        ).expect("alloc q8");
        fix.track(buf)
    };
    let q8_pc = Q8_1QuantizePushConstants { ne: n * k, num_blocks: q8_blocks_x4 as u32 };
    ...
    let out_buf = fix.output_buffer_f32((m * n) as usize, "q5k_out");
    let pc = MmqPushConstants {
        m, n, k,
        stride_a: k,
        stride_b: k,
    ...
    let groups_y = (n + 63) / 64;
    fix.dispatch(
        ShaderId::MulMmqQ5K, &[w_buf, q8_buf, out_buf],
        bytemuck::bytes_of(&pc),
        (groups_x, groups_y, 1),
    );

    let gpu = fix.read_output(out_buf, (m * n) as usize);
    let cpu = cpu_gemm_q5k_ref(&weights, &acts, m as usize, n as usize, k as usize);

    let nan = gpu.iter().any(|x| !x.is_finite());
    let cpu_amax = cpu.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
    let gpu_amax = gpu.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
    eprintln!("[{label}] cpu_amax={cpu_amax:.4e} gpu_amax={gpu_amax:.4e} nan={nan}");
    eprintln!("  CPU first 8: {:?}", &cpu[..8]);
    eprintln!("  GPU first 8: {:?}", &gpu[..8]);

    assert!(!nan, "[{label}] MulMmqQ5K produced NaN/Inf — first 8: {:?}", &gpu[..8]);

    let max_err = max_abs_err(&gpu, &cpu);
    let thresh = (cpu_amax * 0.05).max(0.5);

    if max_err >= thresh {
        eprintln!("[{label}] FAIL — first 8 cols / last 4 cols:");
        let mut col_errs = vec![0.0f32; n as usize];
        let mut col_amax = vec![0.0f32; n as usize];
        for col in 0..n as usize {
            for row in 0..m as usize {
        ...
        for c in 0..(n as usize).min(8) {
            eprintln!("  col {c:>2}: err={:.4e}  cpu_amax={:.4e}",
                col_errs[c], col_amax[c]);
        }
    }
    assert!(
        max_err < thresh,
        "[{label}] MulMmqQ5K vs CPU max_err = {max_err:e} >= {thresh:e}"
    );
    fix.teardown();
}

#[test]
fn test_gemm_q5k_full_tile_64x64_mul_mmq() {
    use vulkanforge::backend::vulkan::q5k;
    run_mul_mmq_q5k_parity(64, 64, q5k::QUANT_K as u32, "M=64 N=64 K=256");
}

#[test]
fn test_gemm_q5k_64x64_k4096_mul_mmq() {
    run_mul_mmq_q5k_parity(64, 64, 4096, "M=64 N=64 K=4096");
}

#[test]
fn test_gemm_q5k_realistic_prefill() {
    // attn_v shape in Qwen3-8B Q3_K_M: kv_dim × hidden = 1024 × 4096.
    // Use M=1024 to match attn_v's m-axis exactly.
    run_mul_mmq_q5k_parity(1024, 32, 4096, "real prefill (attn_v shape)");
}
